...
- (BOOL)play:(BOOL)fullscreen fromPosition:(float)seekPosition
{
    ...
    if ((PLAYING != mediaState || playerType != requestedPlayerType) && NOT_READY > mediaState) {
        if (PLAYER_TYPE_NATIVE == requestedPlayerType) {
            ...
            [[appDelegate arParentViewController] addMoviePlayerToMainView:moviePlayer];
...
- (void)moviePlayerExitAtPosition:(NSTimeInterval)position
{
    ...
    // Remove the MPMoviePlayerController's view
    VideoPlaybackAppDelegate* appDelegate = [[UIApplication sharedApplication] delegate];
    [[appDelegate arParentViewController] removeMoviePlayerView];
    [moviePlayer release];
    moviePlayer = nil;
